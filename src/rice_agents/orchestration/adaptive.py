...
class AdaptiveOrchestrator:
    ...
    async def run(self, goal: str) -> str:
        ...
        for step in plan:
            ...
            if not agent:
                print(f"Error: Agent '{agent_name}' not found in registry. using first available or skipping.")
...
".join(context_accumulator)
    ...
    async def _create_plan(self, goal: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self.manager_llm.chat(messages, system_prompt=system_prompt)
